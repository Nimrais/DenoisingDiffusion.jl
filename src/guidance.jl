...
"""
Matrix label variant: `labels` has shape (C, B) matching batch size.
"""
function p_losses(
    diffusion::GaussianDiffusion,
    loss,
...
function p_sample_loop(
    diffusion::GaussianDiffusion, shape::NTuple, labels::AbstractMatrix{<:Real}
    ; clip_denoised::Bool=true, to_device=cpu, guidance_scale::AbstractFloat=1.0f0
)
    T = eltype(eltype(diffusion))
    x = randn(T, shape) |> to_device
...
function p_sample_loop(diffusion::GaussianDiffusion, labels::AbstractMatrix{<:Real}; options...)
...
function p_sample_loop_all(diffusion::GaussianDiffusion, labels::AbstractMatrix{<:Real}; options...)
    batch_size = size(labels, 2)
    shape = (diffusion.data_shape..., batch_size)
    p_sample_loop_all(diffusion, shape, labels; options...)
end
...
function classifier_free_guidance(
    ...
    # Unconditional labels are zeros(C, B)
    C = size(labels, 1)
